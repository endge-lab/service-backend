-- +goose Up
CREATE TABLE IF NOT EXISTS components_legacy (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    workspace_id UUID NOT NULL REFERENCES workspaces(id),
    project_id UUID NOT NULL,
    folder_id UUID NOT NULL,
    identity TEXT NOT NULL,
    display_name TEXT NOT NULL,
    description TEXT NULL,
    component_type TEXT NOT NULL,
    source TEXT NOT NULL,
    source_format TEXT NOT NULL DEFAULT 'sfc',
    props_schema JSONB NOT NULL DEFAULT '{}'::jsonb,
    bindings JSONB NOT NULL DEFAULT '{}'::jsonb,
    meta JSONB NOT NULL DEFAULT '{}'::jsonb,
    active BOOLEAN NOT NULL DEFAULT TRUE,
    deleted_at TIMESTAMPTZ NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),

    CONSTRAINT components_legacy_workspace_project_fkey
    FOREIGN KEY (workspace_id, project_id)
    REFERENCES projects(workspace_id, id)
    ON DELETE CASCADE,

    CONSTRAINT components_legacy_workspace_folder_fkey
    FOREIGN KEY (workspace_id, folder_id)
    REFERENCES folders(workspace_id, id)
    ON DELETE RESTRICT,

    CONSTRAINT components_legacy_project_identity_unique
    UNIQUE (workspace_id, project_id, identity),

    CONSTRAINT components_legacy_identity_not_empty_check
    CHECK (btrim(identity) <> ''),

    CONSTRAINT components_legacy_display_name_not_empty_check
    CHECK (btrim(display_name) <> ''),

    CONSTRAINT components_legacy_source_not_empty_check
    CHECK (btrim(source) <> ''),

    CONSTRAINT components_legacy_type_check
    CHECK (component_type = 'component-sfc'),

    CONSTRAINT components_legacy_source_format_check
    CHECK (source_format = 'sfc')
);

-- +goose Down
DROP TABLE IF EXISTS components_legacy;
