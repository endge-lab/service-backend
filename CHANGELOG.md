...
- Публичный `GET /version` агрегирует версии подключённых backend-сервисов и безопасно отмечает их недоступность.
- Версия AI Workbench запрашивается через принадлежащий Workbench canonical gRPC contract.
