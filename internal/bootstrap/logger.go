...
func InitLogger(cfg *config.Config, openSearch *openSearchLogExporter) *zap.Logger {
	...
	if openSearch != nil && openSearch.exporter != nil {
		logger = platform.NewLogger(cfg.Logger.Level, "json", "never", cfg.App.Name, cfg.App.Env, cfg.App.Version, openSearch.exporter)
	} else {
		logger = platform.NewLogger(cfg.Logger.Level, "json", "never", cfg.App.Name, cfg.App.Env, cfg.App.Version)
	}
