...
var Definition = documents.Definition{Collection: "vocabs"}

// UseCase координирует сценарии работы с справочниками.
type UseCase struct {
	lifecycle  *documents.Lifecycle
	repository ports.VocabRepository
}

// NewUseCase создаёт use case для работы с справочниками.
func NewUseCase(lifecycle *documents.Lifecycle, repository ports.VocabRepository) *UseCase {
	return &UseCase{lifecycle: lifecycle, repository: repository}
}

// List возвращает документы ресурса «Справочники» с учётом фильтров.
...
// Get возвращает документ ресурса «Справочники» по identity.
...
// Create создаёт документ ресурса «Справочники» и фиксирует новую ревизию.
...
// Patch частично обновляет документ ресурса «Справочники» с проверкой ожидаемой ревизии.
...
// Delete мягко удаляет документ ресурса «Справочники» с проверкой ожидаемой ревизии.
...
// Restore восстанавливает мягко удалённый документ ресурса «Справочники».
