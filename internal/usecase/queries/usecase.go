...
var Definition = documents.Definition{Collection: "queries"}

// UseCase координирует сценарии работы с запросами.
type UseCase struct {
	lifecycle  *documents.Lifecycle
	repository ports.QueryRepository
}

// NewUseCase создаёт use case для работы с запросами.
func NewUseCase(lifecycle *documents.Lifecycle, repository ports.QueryRepository) *UseCase {
	return &UseCase{lifecycle: lifecycle, repository: repository}
}

// List возвращает документы ресурса «Запросы» с учётом фильтров.
...
// Get возвращает документ ресурса «Запросы» по identity.
...
// Create создаёт документ ресурса «Запросы» и фиксирует новую ревизию.
...
// Patch частично обновляет документ ресурса «Запросы» с проверкой ожидаемой ревизии.
...
// Delete мягко удаляет документ ресурса «Запросы» с проверкой ожидаемой ревизии.
...
// Restore восстанавливает мягко удалённый документ ресурса «Запросы».
