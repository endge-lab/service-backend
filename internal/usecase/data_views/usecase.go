...
var Definition = documents.Definition{Collection: "data-views"}

// UseCase координирует сценарии работы с представлениями данных.
type UseCase struct {
	lifecycle  *documents.Lifecycle
	repository ports.DataViewRepository
}

// NewUseCase создаёт use case для работы с представлениями данных.
func NewUseCase(lifecycle *documents.Lifecycle, repository ports.DataViewRepository) *UseCase {
	return &UseCase{lifecycle: lifecycle, repository: repository}
}

// List возвращает документы ресурса «Представления данных» с учётом фильтров.
...
// Get возвращает документ ресурса «Представления данных» по identity.
...
// Create создаёт документ ресурса «Представления данных» и фиксирует новую ревизию.
...
// Patch частично обновляет документ ресурса «Представления данных» с проверкой ожидаемой ревизии.
...
// Delete мягко удаляет документ ресурса «Представления данных» с проверкой ожидаемой ревизии.
...
// Restore восстанавливает мягко удалённый документ ресурса «Представления данных».
