...
var Definition = documents.Definition{Collection: "projects"}

// UseCase координирует сценарии работы с проектами.
type UseCase struct {
	lifecycle  *documents.Lifecycle
	repository ports.ProjectRepository
}

// NewUseCase создаёт use case для работы с проектами.
func NewUseCase(lifecycle *documents.Lifecycle, repository ports.ProjectRepository) *UseCase {
	return &UseCase{lifecycle: lifecycle, repository: repository}
}

// List возвращает документы ресурса «Проекты» с учётом фильтров.
...
// Get возвращает документ ресурса «Проекты» по identity.
...
// Create создаёт документ ресурса «Проекты» и фиксирует новую ревизию.
...
// Patch частично обновляет документ ресурса «Проекты» с проверкой ожидаемой ревизии.
...
// Delete мягко удаляет документ ресурса «Проекты» с проверкой ожидаемой ревизии.
...
// Restore восстанавливает мягко удалённый документ ресурса «Проекты».
