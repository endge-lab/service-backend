...
var Definition = documents.Definition{Collection: "mocks"}

// UseCase координирует сценарии работы с моками.
type UseCase struct {
	lifecycle  *documents.Lifecycle
	repository ports.MockRepository
}

// NewUseCase создаёт use case для работы с моками.
func NewUseCase(lifecycle *documents.Lifecycle, repository ports.MockRepository) *UseCase {
	return &UseCase{lifecycle: lifecycle, repository: repository}
}

// List возвращает документы ресурса «Моки» с учётом фильтров.
...
// Get возвращает документ ресурса «Моки» по identity.
...
// Create создаёт документ ресурса «Моки» и фиксирует новую ревизию.
...
// Patch частично обновляет документ ресурса «Моки» с проверкой ожидаемой ревизии.
...
// Delete мягко удаляет документ ресурса «Моки» с проверкой ожидаемой ревизии.
...
// Restore восстанавливает мягко удалённый документ ресурса «Моки».
