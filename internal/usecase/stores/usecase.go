...
var Definition = documents.Definition{Collection: "stores"}

// UseCase координирует сценарии работы с хранилищами.
type UseCase struct {
	lifecycle  *documents.Lifecycle
	repository ports.StoreRepository
}

// NewUseCase создаёт use case для работы с хранилищами.
func NewUseCase(lifecycle *documents.Lifecycle, repository ports.StoreRepository) *UseCase {
	return &UseCase{lifecycle: lifecycle, repository: repository}
}

// List возвращает документы ресурса «Хранилища» с учётом фильтров.
...
// Get возвращает документ ресурса «Хранилища» по identity.
...
// Create создаёт документ ресурса «Хранилища» и фиксирует новую ревизию.
...
// Patch частично обновляет документ ресурса «Хранилища» с проверкой ожидаемой ревизии.
...
// Delete мягко удаляет документ ресурса «Хранилища» с проверкой ожидаемой ревизии.
...
// Restore восстанавливает мягко удалённый документ ресурса «Хранилища».
