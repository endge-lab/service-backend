...
func TestPatchInputJSONPreservesNullableFields(t *testing.T) {
	t.Parallel()
	input, err := NewPatchInputJSON([]byte(`{"description":null,"startupCompositionIdentity":null,"installedIntegrations":[]}`))
	if err != nil {
	...
	if value, exists := values["startupCompositionIdentity"]; !exists || value != nil {
		t.Fatalf("startupCompositionIdentity = %#v, exists = %v", value, exists)
	}
