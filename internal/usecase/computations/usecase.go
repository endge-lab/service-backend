...
var Definition = documents.Definition{Collection: "computations"}

// UseCase координирует сценарии работы с вычислениями.
type UseCase struct {
	lifecycle  *documents.Lifecycle
	repository ports.ComputationRepository
}

// NewUseCase создаёт use case для работы с вычислениями.
func NewUseCase(lifecycle *documents.Lifecycle, repository ports.ComputationRepository) *UseCase {
	return &UseCase{lifecycle: lifecycle, repository: repository}
}

// List возвращает документы ресурса «Вычисления» с учётом фильтров.
...
// Get возвращает документ ресурса «Вычисления» по identity.
...
// Create создаёт документ ресурса «Вычисления» и фиксирует новую ревизию.
...
// Patch частично обновляет документ ресурса «Вычисления» с проверкой ожидаемой ревизии.
...
// Delete мягко удаляет документ ресурса «Вычисления» с проверкой ожидаемой ревизии.
...
// Restore восстанавливает мягко удалённый документ ресурса «Вычисления».
