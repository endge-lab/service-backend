...
var Definition = documents.Definition{Collection: "streams"}

// UseCase координирует сценарии работы с потоками.
type UseCase struct {
	lifecycle  *documents.Lifecycle
	repository ports.StreamRepository
}

// NewUseCase создаёт use case для работы с потоками.
func NewUseCase(lifecycle *documents.Lifecycle, repository ports.StreamRepository) *UseCase {
	return &UseCase{lifecycle: lifecycle, repository: repository}
}

// List возвращает документы ресурса «Потоки» с учётом фильтров.
...
// Get возвращает документ ресурса «Потоки» по identity.
...
// Create создаёт документ ресурса «Потоки» и фиксирует новую ревизию.
...
// Patch частично обновляет документ ресурса «Потоки» с проверкой ожидаемой ревизии.
...
// Delete мягко удаляет документ ресурса «Потоки» с проверкой ожидаемой ревизии.
...
// Restore восстанавливает мягко удалённый документ ресурса «Потоки».
