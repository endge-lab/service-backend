...
var Definition = documents.Definition{Collection: "environments"}

// UseCase координирует сценарии работы с окружениями.
type UseCase struct {
	lifecycle  *documents.Lifecycle
	repository ports.EnvironmentRepository
}

// NewUseCase создаёт use case для работы с окружениями.
func NewUseCase(lifecycle *documents.Lifecycle, repository ports.EnvironmentRepository) *UseCase {
	return &UseCase{lifecycle: lifecycle, repository: repository}
}

// List возвращает документы ресурса «Окружения» с учётом фильтров.
...
// Get возвращает документ ресурса «Окружения» по identity.
...
// Create создаёт документ ресурса «Окружения» и фиксирует новую ревизию.
...
// Patch частично обновляет документ ресурса «Окружения» с проверкой ожидаемой ревизии.
...
// Delete мягко удаляет документ ресурса «Окружения» с проверкой ожидаемой ревизии.
...
// Restore восстанавливает мягко удалённый документ ресурса «Окружения».
