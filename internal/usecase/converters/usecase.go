...
var Definition = documents.Definition{Collection: "converters"}

// UseCase координирует сценарии работы с конвертерами.
type UseCase struct {
	lifecycle  *documents.Lifecycle
	repository ports.ConverterRepository
}

// NewUseCase создаёт use case для работы с конвертерами.
func NewUseCase(lifecycle *documents.Lifecycle, repository ports.ConverterRepository) *UseCase {
	return &UseCase{lifecycle: lifecycle, repository: repository}
}

// List возвращает документы ресурса «Конвертеры» с учётом фильтров.
...
// Get возвращает документ ресурса «Конвертеры» по identity.
...
// Create создаёт документ ресурса «Конвертеры» и фиксирует новую ревизию.
...
// Patch частично обновляет документ ресурса «Конвертеры» с проверкой ожидаемой ревизии.
...
// Delete мягко удаляет документ ресурса «Конвертеры» с проверкой ожидаемой ревизии.
...
// Restore восстанавливает мягко удалённый документ ресурса «Конвертеры».
