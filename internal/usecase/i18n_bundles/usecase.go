...
var Definition = documents.Definition{Collection: "i18n-bundles"}

// UseCase координирует сценарии работы с пакетами локализации.
type UseCase struct {
	lifecycle  *documents.Lifecycle
	repository ports.I18nBundleRepository
}

// NewUseCase создаёт use case для работы с пакетами локализации.
func NewUseCase(lifecycle *documents.Lifecycle, repository ports.I18nBundleRepository) *UseCase {
	return &UseCase{lifecycle: lifecycle, repository: repository}
}

// List возвращает документы ресурса «Пакеты локализации» с учётом фильтров.
...
// Get возвращает документ ресурса «Пакеты локализации» по identity.
...
// Create создаёт документ ресурса «Пакеты локализации» и фиксирует новую ревизию.
...
// Patch частично обновляет документ ресурса «Пакеты локализации» с проверкой ожидаемой ревизии.
...
// Delete мягко удаляет документ ресурса «Пакеты локализации» с проверкой ожидаемой ревизии.
...
// Restore восстанавливает мягко удалённый документ ресурса «Пакеты локализации».
