...
var Definition = documents.Definition{Collection: "tenants"}

// UseCase координирует сценарии работы с тенантами.
type UseCase struct {
	lifecycle  *documents.Lifecycle
	repository ports.TenantRepository
}

// NewUseCase создаёт use case для работы с тенантами.
func NewUseCase(lifecycle *documents.Lifecycle, repository ports.TenantRepository) *UseCase {
	return &UseCase{lifecycle: lifecycle, repository: repository}
}

// List возвращает документы ресурса «Тенанты» с учётом фильтров.
...
// Get возвращает документ ресурса «Тенанты» по identity.
...
// Create создаёт документ ресурса «Тенанты» и фиксирует новую ревизию.
...
// Patch частично обновляет документ ресурса «Тенанты» с проверкой ожидаемой ревизии.
...
// Delete мягко удаляет документ ресурса «Тенанты» с проверкой ожидаемой ревизии.
...
// Restore восстанавливает мягко удалённый документ ресурса «Тенанты».
