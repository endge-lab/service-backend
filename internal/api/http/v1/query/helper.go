...
func (h *Handler) response(value *adapters.QueryWithFolder, projectIdentity string) *QueryResponse {
	if value == nil {
		return nil
	}
...
func newQueryResponse(value *entities.Query, projectIdentity, folderIdentity string) *QueryResponse {
	if value == nil {
		return nil
	}
...
func (h *Handler) change(c *fiber.Ctx, fn func(context.Context, adapters.QueryIdentityInput) error) error {
	if err := fn(c.UserContext(), adapters.QueryIdentityInput{ProjectIdentity: c.Params("project_identity"), QueryIdentity: c.Params("query_identity")}); err != nil {
